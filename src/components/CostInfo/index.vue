<template>
  <div class="">
    <el-dialog :visible.sync="visible" width="80%" @close="close">
      <div slot="title" class="dialog-title">
        <span style="color: #606266;margin-left:20px">费用信息 </span>&nbsp;&nbsp;&nbsp;&nbsp;
        <span style="font-weight:700;color:blue">运单编号：{{ vbillno }}</span>
      </div>
          ...
          >
            <template slot-scope="scope">
              <span v-if="scope.row.isSet">
                ...
                <el-checkbox v-else-if="i===2" v-model="payList.sel[v.field]" true-label="Y" false-label="N"/>
                <el-input-number v-else-if="i===3" v-model="payList.sel[v.field]" :min="0" :max="99999999" :step="0.01" :precision="2"/>
...
export default {
    ...
    return {
      ...
      payList: {
        ...
        columns: [
          ...
          { field: 'sub_fee', title: '是否代付', width: '40px' },
  ...
  methods: {
    ...
    editRevenueOrPay(row, index, cg, item) {
      ...
      if (row.isSet) {
        ...
        const msg = {
          ...
          invoice_vbillno: this.orderForm.invoice_vbillno,
          deli_address: this.orderForm.deli_address,
          entrust_vbillno: this.vbillno,
          memo: item.sel.memo,
    ...
    getCostInfo(item) {
      this.vbillno = item
      const data = {
        entrust_vbillno: item,
        userCode: '15001962939'
      }
      costInfo(data).then(response => {
        // console.log(response.data.data)
        this.orderForm.vbillno = response.data.data.vbillno
        // this.vbillno = response.data.data.vbillno
        ...
        // for (const item of this.payList.data) {
        //   if (item.pay_type === 1) {
        //     item.pay_type = '电汇'
        //   } else if (item.pay_type === 2) {
        //     item.pay_type = '现金'

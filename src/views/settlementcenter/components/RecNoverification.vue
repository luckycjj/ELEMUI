<template>
  <div>
    ...
    <div class="page-operation-bottons">
      <el-row>
        <!-- <el-button v-if="checkPermission(['tms.settlement.collect.unverifyExport'])||checkPermission(['tms.settlement.pay.unverifyExport'])" class="port-btn" icon="el-icon-download" @click="exportList">导出</el-button> -->
        <el-button v-if="checkPermission(['tms.settlement.collect.unverifyDue'])||checkPermission(['tms.settlement.pay.unverifyDue'])" class="change-btn" icon="fa fa-upload" @click="addDialog"> 收款核销</el-button>
    ...
    <el-dialog
      v-if="checkPermission(['tms.settlement.collect.unverifyDue'])||checkPermission(['tms.settlement.pay.unverifyDue'])"
...
import { checkList, choosepkbala, lookCheck, ReceCheck } from '@/api/settlement'
  ...
  methods: {
    ...
    addDialogs(number) {
      ...
      lookCheck({
        ...
        _this.$refs.soas.list = response.data.backFeeDtoList

<template>
  <div>
    ...
    <div class="page-operation-bottons">
      <el-row>
        <el-button v-if="checkPermission(['tms.settlement.collect.verifiedExport'])||checkPermission(['tms.settlement.pay.verifiedExport'])" class="port-btn" icon="el-icon-download" @click="exportList" >导出</el-button>
...
export default {
  name: 'RecVerification',
  components: { Pagination, SoaDialogs },
  directives: { waves },
  data() {
    return {
      source: 0,
      ...
      listQuery: {
        ...
        type: 4, // 1未对账、2未开票、3未核销、4已核销、5全部
        account_period: '', // 账期
        check_vbillno: '',
        bala_customer: '', // 结算单位
        bala_type: '', // 结算方式

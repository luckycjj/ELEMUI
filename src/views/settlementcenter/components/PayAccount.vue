  ...
  <div>
    ...
    <div class="page-operation-bottons">
      <el-row>
        ...
        <el-button v-if="checkPermission(['tms.settlement.pay.ensure'])" class="confrim-btn" icon="el-icon-success" @click="sureOrder">确认</el-button>
        <el-button v-if="checkPermission(['tms.settlement.pay.delete'])" class="cancel-btn" icon="el-icon-delete" @click="deleteOrder">删除对账单</el-button>
      </el-row>
    </div>
  </div>
</template>
...
import { lookCheck, PayCheckCheckList, ReceCheckConfirmCheck, ReceCheckDeleteCheckHead } from '@/api/settlement'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      PayCheckCheckList(this.listQuery).then(response => {

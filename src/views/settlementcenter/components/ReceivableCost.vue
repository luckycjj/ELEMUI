<template>
  <div>
    <div class="filter-container">
      ...
      <!-- <el-input v-model="listQuery.bala_customer" placeholder="结算客户" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter"/> -->
      <el-date-picker
    ...
    <invoice ref="revInvoice" :show.sync="dialogFormVisible" :onselect="selectInvoice" :opinion.sync="invoiceInfo"/>
    <div class="page-operation-bottons">
      <el-row>
        <el-button v-if="checkPermission(['tms.settlement.costlist.ticket '])" class="change-btn" icon="fa fa-upload" @click="addInvocie"> 生成发票</el-button>
...
export default {
  ...
  methods: {
    ...
    addInvocie() {
      ...
      var list = []
      for (var i = 0; i < vbillno.length; i++) {
        if (list.indexOf(vbillno[i]) === -1) {
          list.push(vbillno[i])
        }
      }
      this.invoiceInfo = {
      ...
      // this.$refs.revInvoice.invoiceForm.bill_amount = this.numTotal

...
<template>
  <div>
    ...
    <el-table
      ...
      <el-table-column prop="segment_vbillno" label="托单号" min-width="200"/>
      ...
      <el-table-column prop="pk_delivery" label="提货方" min-width="150"/>
      ...
      <el-table-column fixed="right" label="操作" min-width="100">
        <template slot-scope="scope">
          ...
          <span v-if="checkPermission(['tms.waybill.cost'])" class="link-type" @click="costDetail(scope.row.vbillno,scope.row.vbillstatus)">费用</span>
        </template>
      </el-table-column>
    </el-table>
    ...
    <div class="page-operation-bottons">
      <el-row>
        <el-button class="port-btn" icon="el-icon-download" @click="exportEntrust">导出</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      entrustStateOptions: EnumType.entrustStates,
  ...
  mounted() {
    ...
    window.onresize = () => {
      ...
      return (() => {
        ...
        that.innerHeight = document.body.clientHeight
      })()
    }
  },
  methods: {
    ...
    exportEntrust() {
      exportEntrust(this.listQuery).then(response => {
        const fileName = '全部运单' + moment(new Date()).format('YYYYMMDDhhmmss') + '.xls'
        // 获取文件名

...
<template>
  <div>
      ...
      <el-table-column fixed="right" label="操作" min-width="140" align="center">
        <template slot-scope="scope">
          ...
          <el-row v-else >
            ...
            <el-button v-if="checkPermission(['tms.order.cost'])" type="text" @click="editReceCost(scope.row.vbillno,scope.row.vbillstatus)">费用</el-button>
          </el-row>
        </template>
      </el-table-column>
    </el-table>
    ...
    <div class="page-operation-bottons">
      <el-row>
        <!-- <el-button type="primary" icon="el-icon-upload2" @click="dialogUploadExcel=true">导入</el-button> -->
        <el-button v-if="checkPermission(['tms.order.all.export'])" class="port-btn" icon="el-icon-download" @click="exportInvoice">导出</el-button>
        <!-- <el-button class="port-btn" icon="el-icon-download" @click="downloadZip">回单导出</el-button> -->
    ...
    <!-- <el-dialog :visible.sync="dialogUploadExcel" title="导入订单" width="400px">
      <upload-excel-component ref="uploadExcel"/>
...
export default {
  name: 'ShippedOrder',
  components: { Pagination, LookAttachment, UploadExcelComponent, ReceCost },
  directives: { waves },
  data() {
    return {
      tableKey: 0,
      ...
      multipleSelection: [],
      vbillstatusOptions: EnumType.vbillstatusAllOptions,
  ...
  mounted() {
    ...
    that.$nextTick(function() {
      that.innerHeight = (document.documentElement.clientHeight || document.body.clientHeight) - 410
      window.onresize = () => {
        return (() => {
          that.innerHeight = (document.documentElement.clientHeight || document.body.clientHeight) - 410
        })()
  ...
  methods: {
    ...
    exportInvoice() {
      exportInvoice(this.listQuery).then(response => {
        const fileName = '全部订单' + moment(new Date()).format('YYYYMMDDhhmmss') + '.xls'
        // 获取文件名

...
import { getToken, setToken, removeToken, removeSysCode, removeUserCode } from '@/utils/auth'
    ...
    GetUserInfo({ commit, state }) {
      ...
      return new Promise((resolve, reject) => {
        getUserInfo(state.token).then(response => {
          ...
          // debugger
          const data = response.data.data
          commit('SET_NAME', data.currentUserVo.user_name)
          commit('SET_COMPANY', data.currentUserVo.pk_customer)
          commit('SET_CODE', data.currentUserVo.user_code)
          commit('SET_AVATAR', 'https://wpimg.wallstcn.com/f778738c-e4f8-4870-b634-56703b4acafe.gif')
          commit('SET_INTRODUCTION', data.currentUserVo.user_name)
          resolve(response)
        }).catch(error => {
          reject(error)
        })

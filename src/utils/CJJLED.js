(function() {
  ...
  LEDCJJ.prototype = {
    init: function() {
      ...
      var len = (_this.arr.number.toString()).length
      var dom = document.getElementById(_this.arr.dom)
      dom.innerHTML = ''
      var childdom = ''
      var re = /^[0-9]+.?[0-9]*$/
      for (var i = 0; i < len; i++) {
        if (re.test(_this.arr.number.toString()[i])) {
    ...
    domList: function() {
      ...
      var domBox = "<div class='LEDLineBox'  style='position: relative;float: left;width:" + width + 'px ;height: ' + width * 1.9 + 'px; margin: 0 ' + width / 8 + "px;'>" + str + '</div>'
      return domBox
    },
    notNumber: function() {
      var _this = this
      ...
      var domBox = "<div class='LEDLineBox' style='position: relative;float: left;width:" + width / 4 + 'px ;height: ' + width * 1.9 + 'px; margin: 0 ' + width / 20 + "px;' >" + str + '</div>'
      return domBox
    },
    pushColor: function() {
      var _this = this
      ...
      for (var i = 0; i < len; i++) {
        var number = (_this.arr.number.toString())[i]
        for (var x = 0; x < _this.numberList.length; x++) {
          if (number * 1 === _this.numberList[x].number * 1) {
